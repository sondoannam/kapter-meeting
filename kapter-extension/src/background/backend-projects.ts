...
export async function fetchProjects(
  ...
  try {
    response = await fetchImpl(new URL("/api/projects", `${apiBaseUrl}/`), {
      method: "GET",
      headers: {
    ...
    throw new Error(`Unable to load projects: ${message}`);
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      message
        ...
        : `Unable to load projects with status ${response.status}.`,
    );
  }

  return (await response.json()) as BackendProjectsResponse;
}
