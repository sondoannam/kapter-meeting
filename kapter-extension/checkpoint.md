...
- [x] 2026-05-13 Added a lightweight webapp-origin presence response in the content script, so the guided dashboard can detect that the extension is injected on the current tab without involving the background worker or auth flow.
- [x] 2026-05-16 Added a fresh Google Meet mic-state preflight at recorder start and made `self_mic` streaming default-deny unless Meet is explicitly unmuted, preventing stale or unknown mute-state leaks to the backend.
- [x] 2026-05-17 14:48 +07:00 Promoted the maintained extension auth spec into an official `pnpm test` path, kept packaging verification as a separate maintained `test:package` path, and re-enabled both commands in extension CI and the aggregate validation report.
