...
- **Typescript contracts.** For shared data structures, use the existing `@kapter/contracts` package. Do not create new contract files without user approval, and do not modify existing contracts without explicit instruction. Update contracts first, then implement changes in other modules to match. Don't use modules-local type aliases for shared structures.
