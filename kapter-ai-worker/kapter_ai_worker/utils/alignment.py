...
def strip_overlap(base_text: str, new_text: str, max_words: int = 50) -> str:
    ...
    for i in range(len(history_slice) - anchor_size, -1, -1):
        ...
        match_count = sum(1 for j in range(anchor_size) if history_slice[i+j] == anchor[j])
...
def consolidate_segments(
    ...
    if len(resolved_segments) >= 2:
        for i in range(len(resolved_segments)):
            ...
            if duration < 0.8:
                ...
                next_seg = resolved_segments[i+1] if i < len(resolved_segments) - 1 else None
                
                if prev_seg and next_seg and prev_seg.speaker_label == next_seg.speaker_label:
    ...
    for i in range(1, len(resolved_segments)):
        ...
        if is_same_speaker and gap <= max_gap_seconds:
            ...
            current.text = f"{base_text} {new_text_cleaned}"
