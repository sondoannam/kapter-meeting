...
- [x] 2026-05-17 Standardized the worker test entrypoint on the repo-local `.venv` via `scripts/run_tests.py`, redirected pytest temp state into module-local `.pytest-runs`, and re-enabled the maintained worker suite in CI and the aggregate validation report.
