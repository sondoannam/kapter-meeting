...
- [x] 2026-05-09 Added the subscription quota foundation with Prisma `Subscription`/`UsageLedger` models, public/authenticated billing APIs, quota enforcement before `stream:start`, and post-meeting usage recording that avoids double-counting dual-lane audio.
- [x] 2026-05-09 Fixed BillingModule startup wiring by importing ClerkModule so BillingService can resolve PrismaService when the backend boots.
- [x] 2026-05-09 Added backend metadata-edit readiness for the dashboard by extending project updates to include stored context fields and introducing a dedicated meeting metadata patch path for title, description, external reference, and pre-approval project reassignment.
- [x] 2026-05-09 Added authenticated project deletion for empty projects only, with a linked-meeting conflict guard plus focused controller/service coverage so the dashboard can surface truthful delete UX instead of unsafe hard deletes.
- [x] 2026-05-09 Replaced the empty-project delete guard with transactional cascade deletion of linked meetings and added an authenticated meeting delete endpoint for dashboard and meeting-detail destructive actions.
- [x] 2026-05-09 Moved dashboard meeting/project response and request DTOs into `@kapter/contracts`, updated meetings/projects services to consume the shared contract types, and corrected the project delete endpoint summary to match cascade-delete behavior.
- [x] 2026-05-14 Implemented the backend half of the voice profile MVP with Prisma `VoiceProfile`/sample persistence, AI-worker cache sync + enrollment APIs, meeting speaker link/promote/clear flows, first-batch known-profile seeding, and passing backend typecheck plus focused service-spec validation.
- [x] 2026-05-14 Productionized the cloud Postgres voice-profile path by adding `DIRECT_URL` support for Prisma/admin tasks, a backend-driven full worker cache rebuild flow plus startup warmup, a manual `voice-profiles:rebuild-worker-cache` maintenance script, and updated managed-Postgres rollout docs.
- [x] 2026-05-14 Added recurring unknown-speaker memory and conservative post-meeting short-turn repair, including new Prisma models/status fields, completed-meeting post-processing orchestration, advisory recurring-speaker metadata on meeting detail, Prisma migration `20260514144431_recurring_unknown_speaker_memory`, and passing backend typecheck plus focused meeting service/spec coverage.
- [x] 2026-05-16 Added dashboard MP3 meeting ingestion with authenticated multipart upload acceptance, `FILE_UPLOAD` meeting persistence plus Prisma migration `20260516120000_add_meeting_ingestion_source`, temp-file staging and cleanup, async AI-worker file processing, upload quota billing, startup reconciliation for stranded uploads, and focused controller/service/client validation.
- [x] 2026-05-16 Fixed uploaded-audio worker timeouts by giving `process-audio-file` its own longer backend timeout budget, preserving the shorter live-batch timeout while preventing 30-second aborts against multi-minute offline processing.
- [x] 2026-05-17 Added MinIO-ready meeting media storage for uploaded recordings, including startup bucket bootstrap config, shared backend media storage service, durable media retention after successful processing, stored-audio cleanup on failed-upload expiry and meeting/project deletion, and focused backend validation.
- [x] 2026-05-17 14:48 +07:00 Stabilized backend validation for the monorepo test report by replacing the fragile `tsx --test` path with a compiled Node test runner, preloading local Prisma-client resolution for compiled specs, wiring DB-backed audio integration specs to skip cleanly when local Postgres is unavailable, promoting maintained backend tests back into CI, and keeping lint scoped to source instead of `.test-dist` artifacts.
- [x] 2026-05-17 Hardened uploaded-meeting transcript persistence so stale worker `voiceProfileId` references and malformed speaker-evidence numeric values no longer fail the entire meeting, and added clearer upload failure stack logging plus focused persistence-spec coverage.
