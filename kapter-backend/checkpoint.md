...
- [x] 2026-05-06 19:46:01 +07:00 Bypassed Clerk auth for HTTP OPTIONS preflight requests and added a guard regression test so deployed webapp CORS preflights no longer fail early with `Missing Clerk session token.`
- [x] 2026-05-06 20:25:33 +07:00 Hardened backend URL config parsing to strip wrapping quotes and trailing slashes from `CORS_ORIGIN`, `CLERK_AUTHORIZED_PARTIES`, and `WEBAPP_BASE_URL`, after production responses showed missing `Access-Control-Allow-Origin` despite successful preflight routing.
