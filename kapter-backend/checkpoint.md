...
- [x] 2026-05-05 19:11 +07:00 Added deployment-ready backend worker auth config (`AI_WORKER_BASE_URL`, timeout, shared secret) and a Heroku-targeted monorepo container image (`Dockerfile.heroku`) for the MVP backend hosting path.
- [x] 2026-05-05 23:58 +07:00 Aligned backend deployment docs with the final MVP hosting topology by adding `WEBAPP_BASE_URL` and production Heroku/Vercel/worker examples to `kapter-backend/.env.example`.
