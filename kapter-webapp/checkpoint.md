...
- [x] 2026-05-11 Replaced the custom dashboard sidebar shell with shadcn/ui sidebar primitives, removed the old observer-based positioning code, added a dashboard-scoped pricing route, and validated the refactor with passing `pnpm --filter ./kapter-webapp typecheck` plus a passing escalated `pnpm --filter ./kapter-webapp build` run; `pnpm --filter ./kapter-webapp lint` still fails on pre-existing issues in `src/components/ui/button.tsx`, `src/components/ui/sidebar.tsx`, and `src/hooks/use-mobile.ts`.
- [x] 2026-05-12 Added a guided beginner dashboard mode on `/dashboard`, persisted guided/standard mode locally with early-stage coachmark behavior plus automatic graduation after first review approval, and validated the rollout with passing `pnpm --filter ./kapter-webapp typecheck`, passing `pnpm --filter ./kapter-webapp lint` aside from the existing fast-refresh warnings in `src/components/ui/button.tsx` and `src/components/ui/sidebar.tsx`, and a passing escalated `pnpm --filter ./kapter-webapp build`.
- [x] 2026-05-13 Added a guided-dashboard extension presence handshake on the webapp side, so `/dashboard` can auto-detect the injected extension on the current tab, advance from install to capture without persisting that detection, and preserve the existing manual fallback plus fresh-install copy.
- [x] 2026-05-14 Added the user-owned voice profile MVP UI with dashboard management, enrollment upload, meeting-speaker link/promote/clear controls, and meeting detail refresh wiring, validated by a passing `pnpm --filter ./kapter-webapp typecheck`.
- [x] 2026-05-14 Extended the meeting speaker roster with advisory recurring-speaker hints from backend post-processing, keeping the existing mapping flow unchanged and validating with a passing `pnpm --filter ./kapter-webapp typecheck`.
- [x] 2026-05-15 Added page-level skeleton loading states for all data-fetching pages: replaced `AppLoadingScreen` full-screen spinner with inline skeleton layouts in meeting-detail and voice-profiles, added a new skeleton to pricing (previously had none), and expanded the dashboard skeleton to cover the full 2-column workspace layout. Validated with a passing `pnpm --filter ./kapter-webapp typecheck`. (2026-05-15 09:29 +07:00)
- [x] 2026-05-15 Added multilingual support to the pricing page with a dedicated `pricing` i18n namespace, localized plan/quota copy, and locale-aware number/date formatting. (2026-05-15 21:46 +07:00)
- [x] 2026-05-15 Reworded the pricing quota-protection callout and billing error fallbacks into user-facing copy without backend/stream implementation details. (2026-05-15 22:26 +07:00)
- [x] 2026-05-16 Added dashboard and guided-mode MP3 upload entry points with a shared upload dialog, default title/project handling, immediate meeting-detail handoff after acceptance, upload-aware active-session messaging, and refreshed English/Vietnamese empty-state copy.
